...
public class PageMaker {
	...
	private int endPage; //마지막 페이지 번호(현제 페이지네이션에서)
	...
	public void calcData() {
		...
		//endPage = (int) (Math.ceil(6/10.0)*10); = 10
		startPage = (endPage - displayPageNum)+1;
		/* 총 콘텐츠 갯수를 이용하여 마지막 페이지 번호를 계산 */
		//tempEndPage : 마지막 페이지 번호(전체 페이지네이션 중에서)
		int tempEndPage = (int)(Math.ceil(totalCount/(double)cri.getPerPageNum()));
		//int tempEndPage = (int)(Math.ceil(101/10.0)); = 11
		...
		//prev = criteria.getPage() == 1 ? false : true; //현재 페이지가 첫번째 페이지면 이전버튼 비활성화
		...
		//next = criteria.getPage() == endPage ? false : true; //현재 페이지가 마지막 페이지면 다음버튼 비활성화
	}
